...
//    click_chatter("ProcessAdvertisement::push");
    click_ip* iph = (click_ip*) packet->data();
    if (iph->ip_p == 1) {
//        click_chatter("ICMP found");
...
//                        click_chatter("Add advertisement to MN");
...
//    click_chatter("ProcessAdvertisement::run_timer");
    IPAddress address;
    Packet* packet;
    for (int i = 0; i < _timers.size(); i++) {
    ...
    if (lifetime > 0) {
        ...
        if (p != 0) {
//            click_chatter("no advertisement found");
            return;
        }
        p->value->kill();
