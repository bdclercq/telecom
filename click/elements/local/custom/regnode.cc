...
//    click_chatter("RegNode::push");
    ...
    if(!found) {
//        click_chatter("No matching pending request found");
        p->kill();
        return;
    }
...
//    click_chatter("Check to code of the request");
    if (code == 0 || code == 1) {
//        click_chatter("WE'RE IN");
        _timer.clear();
        if (iph->ip_src != _mninfo->_gateway) {
//            click_chatter("EH...");
